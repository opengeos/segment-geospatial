...
def main():
    ...
    try:
        ...
        print("   - Clay dependencies: pip install claymodel torch torchvision pyyaml python-box")
        return
    ...
    try:
        ...
        if embeddings.shape[0] > 0:
            ...
            embedding_2d = first_embedding[:256].reshape(16, 16)  # Take first 256 values
            axes[1].imshow(embedding_2d, cmap='viridis')
            axes[1].set_title('Clay Embedding Visualization\n(First 256 features, first tile)')
            axes[1].axis('off')
...
def example_with_numpy_array():
    ...
    try:
        ...
        clay = ClayEmbeddings(
            checkpoint_path="path/to/clay-model-checkpoint.ckpt",
            device="auto"
        )
