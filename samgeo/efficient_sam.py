...
class SamGeo:
    """The main class for segmenting geospatial data with the Efficient Segment Anything Model (SAM). See
    https://github.com/yformer/EfficientSAM for details.
    """

    def __init__(
        self,
        model_type="efficientsam_s_gpu.jit",
        automatic=False,
        device=None,
        checkpoint_dir=None,
        sam_kwargs=None,
        **kwargs,
        ...
        if model_type not in ["efficientsam_s_gpu.jit", "efficientsam_ti_gpu.jit"]:
            raise ValueError(
                "Invalid model type. It can be one of the following: efficientsam_s_gpu.jit, vitefficientsam_ti_gpu.jit"
            )

        checkpoint = os.path.join(checkpoint_dir, model_type)

        if not os.path.exists(checkpoint):
            url = f"https://github.com/opengeos/data/releases/download/models/{model_type}"
