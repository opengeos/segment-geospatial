...
def orthogonalize(
    ...
    def align(df):
        ...
        df_clone["x1"] = df_clone["cx"] - (
            (df_clone["len"] / 2) * np.cos(df_clone["angle"])
        )
        df_clone["x2"] = df_clone["cx"] + (
            (df_clone["len"] / 2) * np.cos(df_clone["angle"])
        )
        df_clone["y1"] = df_clone["cy"] - (
            (df_clone["len"] / 2) * np.sin(df_clone["angle"])
        )
        df_clone["y2"] = df_clone["cy"] + (
            (df_clone["len"] / 2) * np.sin(df_clone["angle"])
        )
